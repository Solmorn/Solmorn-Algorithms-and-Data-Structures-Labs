...
int main(void) {
    ...
    for (int i = 0; i < TESTS_IN_SERIE; i++) {
        int* data_test_1 = CreateData(5000000);
        if (!data_test_1) return 1;
    ...
    test_arr_stack_time  = 0;
    test_list_stack_time = 0;

    for (int i = 0; i < TESTS_IN_SERIE; i++) {
        int* data_test_2 = CreateData(5000000);
        if (!data_test_2) return 1;
    ...
    for (int i = 0; i < TESTS_IN_SERIE; i++) {
        int* data_test_3_values = CreateData(5000000);
        int* data_test_3_instructions = CreateData(1000000);

        if (!data_test_3_values || !data_test_3_instructions) {
    ...
    if (!TestFour_both_stack("graph_data.txt")) return 1;
